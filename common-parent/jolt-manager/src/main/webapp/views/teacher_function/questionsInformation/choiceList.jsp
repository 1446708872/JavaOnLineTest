...
<script>
    ...
    function queryop(data) {
        ...
        for(var i=0;i<data.length;i++){
            ...
            list+=' <tr>\n' +
                ...
                '                <td style="text-align:center;"><button style="width: 100px" id="'+data[i].choice_id+'" onclick="choiceSee(this)" type="button" >查看试题</button></td>\n' +
                '                <td style="text-align:center;"><button style="width: 100px" id="'+data[i].choice_id+'" onclick="choiceUpdate(this)" type="button" >修改试题</button></td>\n'+
                '                <td style="text-align:center;"><button style="width: 100px" id="'+data[i].choice_id+'" onclick="deleteSize(this)" type="button" >删除试题</button></td>\n' +
                '</tr>'
        }
    ...
    function deleteSize(obj) {
        $.ajax({
            url : '${pageContext.request.contextPath}/lib/deleteChoice.do',
            type : "post",
    ...
    function choiceSee(obj) {
        $("#hidden").val(obj.id)
        //加载所点击功能模块
        $.get("../views/teacher_function/questionsInformation/choiceList/choiceSee.jsp",function(data){
            $("#mian").html(data);
        });
    }
    ...
    function choiceUpdate(obj) {
        $("#hidden").val(obj.id)
        //加载所点击功能模块
        $.get("../views/teacher_function/questionsInformation/choiceList/choiceUpdate.jsp",function(data){
            $("#mian").html(data);
        });
    }
</script>
